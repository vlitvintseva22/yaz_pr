...
Формат (комментарии начинаются с #, пустые строки игнорируются):
...
Скорости в файле НЕ хранятся: файл описывает геометрию, а скорости
пересчитываются при загрузке из условия круговой орбиты (set_circular_orbit).
"""
