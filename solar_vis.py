...
# Шрифты грузим НАПРЯМУЮ из файла, а НЕ через pygame.font.SysFont.
# SysFont перебирает шрифты в реестре Windows, и на некоторых машинах
# падает с «expected str ... not int» (битая запись реестра типа REG_DWORD).
# Эти системные шрифты есть на любой Windows и содержат кириллицу.
_FONT_CANDIDATES = (
...
def get_font(size=14, bold=False):
    """
    Возвращает кэшированный шрифт нужного размера, загруженный НАПРЯМУЮ из
    файла (минуя SysFont и его перебор реестра, который падает на некоторых
    машинах). Берёт первый существующий системный шрифт с кириллицей; если
    ни один не найден — резервный встроенный шрифт pygame (без кириллицы).
    """
    key = (size, bold)
